...
async function versionHistoryDemo() {
    console.log(`${colors.blue}${colors.bright}🔄 Version History & Rollback Demo${colors.reset}`);
    console.log("Demonstrating SPEC: Version immutability, history tracking, and rollback workflow\n");
    ...
    try {
        ...
        console.log(`${colors.cyan}Step 1: Creating project for version tracking...${colors.reset}`);
        
        const collection = await Effect.runPromise(
            storage.createCollection("Version Demo", "Collection for demonstrating version control")
        );
...
*Version 1.0 - Initial release*`;
        ...
        console.log(`${colors.green}✅ Version 1.0 published:${colors.reset}`);
...
*Version 1.1 - Enhanced requirements and quality standards*`;
        ...
        await Effect.runPromise(
            storage.saveDraft({
                ...
                metadata: {
                    ...
                    version: "1.1",
                    status: "published",
                    changes: ["enhanced requirements", "quality standards", "UX review process"]
                }
            })
        );
        ...
        console.log(`${colors.green}✅ Version 1.1 published:${colors.reset}`);
        console.log(`   Version ID: ${v1_1Publication.version_id}`);
        console.log(`   Content: Enhanced with quality standards and UX requirements`);
...
*⚠️ NOT YET APPROVED FOR PRODUCTION USE*`;
        ...
        await Effect.runPromise(
            storage.saveDraft({
                ...
                metadata: {
                    ...
                    changes: ["AI integration", "new format", "breaking changes", "revolutionary approach"]
                }
            })
        );
        ...
        console.log(`${colors.yellow}⚠️ Version 2.0 published (EXPERIMENTAL):${colors.reset}`);
        console.log(`   Version ID: ${v2Publication.version_id}`);
        console.log(`   Content: Experimental AI-enhanced approach with breaking changes`);
        ...
        console.log(`${colors.green}✅ Complete version history (${versionHistory.length} versions):${colors.reset}`);
        versionHistory.forEach((version, index) => {
            ...
            if (version.parent_version_id) {
                console.log(`     Parent: ${version.parent_version_id.substring(0, 16)}...`);
            }
            console.log('');
        });

        // Step 6: Demonstrate rollback (SPEC: Creates new Version referencing target)
        console.log(`${colors.cyan}Step 6: Rolling back to stable version (v1.1)...${colors.reset}`);
        console.log(`${colors.yellow}Reason: v2.0 experimental features not ready for production${colors.reset}`);
        
        const targetVersion = versionHistory.find(v => v.content_md.includes('v1.1'));
        if (!targetVersion) {
        ...
        console.log(`${colors.green}✅ Rollback verification:${colors.reset}`);
        console.log(`   Total versions: ${postRollbackHistory.length} (was ${versionHistory.length})`);
        console.log(`   New version references target: ${rollbackVersion.parent_version_id === targetVersion.id ? 'Yes' : 'No'}`);
        console.log(`   Current version content: ${rollbackVersion.content_md.split('\n')[0]}`);
        console.log(`   Content matches v1.1: ${rollbackVersion.content_md.includes('v1.1') ? 'Yes' : 'No'}`);

        // Step 8: Show final version tree
        console.log(`\n${colors.cyan}Step 8: Final version tree visualization...${colors.reset}`);
        
        console.log(`${colors.green}✅ Version evolution tree:${colors.reset}`);
        console.log(`   v1.0 (initial) → v1.1 (enhanced) → v2.0 (experimental)`);
        console.log(`                                  ↘`);
        console.log(`                                    v1.1-rollback (current)`);
        console.log(`                                    ↑ references v1.1`);

        console.log(`\n${colors.magenta}📋 Version metadata comparison:${colors.reset}`);
        postRollbackHistory.slice(0, 4).forEach((version, index) => {
            const versionNum = index === 0 ? 'CURRENT' : `v-${index}`;
            console.log(`   ${versionNum}: ${version.label} | ${version.created_at.toDateString()}`);
        });

        // Summary with SPEC compliance
        console.log(`\n${colors.yellow}${colors.bright}📊 Version History Demo Summary:${colors.reset}`);
        console.log(`• Created ${postRollbackHistory.length} versions across 3 major iterations`);
        console.log(`• Demonstrated version immutability (each change creates new version)`);
        ...
        console.log(`• Rollback creates new Version referencing target ✅`);
        console.log(`• Parent relationships preserved ✅`);
        console.log(`• Complete version history maintained ✅`);
        console.log(`• Version labels (minor/major) tracked ✅`);
        ...
        console.error(`${colors.red}❌ Version history demo failed:${colors.reset}`, error);
        throw error;
    } finally {
...
async function main() {
    try {
        ...
        console.log(`\n${colors.green}🎉 Version history demo completed successfully!${colors.reset}`);
        console.log(`${colors.cyan}Tracked ${result.finalVersions.length} versions with complete rollback workflow${colors.reset}`);
    } catch (error) {
