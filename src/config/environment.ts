...
const ConfigSchema = Schema.Struct({
	...
	observability: Schema.Struct({
		...
		retentionDays: Schema.Number.pipe(Schema.int(), Schema.greaterThanOrEqualTo(0)),
	}),
	rateLimits: Schema.Struct({
		queries: Schema.Struct({
			burstPerSecond: Schema.Number.pipe(Schema.int(), Schema.greaterThan(0)),
			sustainedPerMinute: Schema.Number.pipe(Schema.int(), Schema.greaterThan(0)),
		}),
...
const candidateConfig = {
	nodeEnv,
	server: {
		port: parseNumber(rawEnv.SERVER_PORT ?? rawEnv.PORT, defaults.server.port, "SERVER_PORT"),
	},
	database: {
	...
	features: {
		usePostgres: parseBoolean(rawEnv.USE_POSTGRES, defaults.features.usePostgres),
		autoMigrate: parseBoolean(rawEnv.AUTO_MIGRATE, defaults.features.autoMigrate),
	},
	observability: {
