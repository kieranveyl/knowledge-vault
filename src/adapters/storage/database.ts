...
export type DatabaseError =
	...
	| { readonly _tag: "MigrationFailed"; readonly migration: string; readonly reason: string }
	| { readonly _tag: "QueryFailed"; readonly query: string; readonly reason: string }
		...
		Effect.promise(async () => {
			try {
				...
				throw {
					_tag: "QueryFailed",
					query: text,
					reason: error instanceof Error ? error.message : "Unknown query error",
				};
			}
		});
	...
	readonly transaction = <T>(
		...
		Effect.promise(async () => {
			...
			try {
				...
				const transactionQuery = <R = any>(text: string, params?: any[]): Effect.Effect<R[], DatabaseError> =>
					Effect.promise(async () => {
						try {
							...
							throw {
								_tag: "QueryFailed",
								query: text,
								reason: error instanceof Error ? error.message : "Transaction query failed",
							};
						}
					});
...
export class MigrationManager {
	 ...
	 */
	readonly getAppliedMigrations = (): Effect.Effect<readonly Migration[], DatabaseError> =>
	 ...
	 */
	readonly runMigrations = (): Effect.Effect<{ applied: readonly string[] }, DatabaseError> =>
		Effect.gen(this, function* () {
				...
				if (!appliedNames.has(migrationName)) {
					try {
						...
						yield* Effect.fail({
							...
							reason: error instanceof Error ? error.message : "Failed to read migration file",
						} as DatabaseError);
					}
				}
			}
