...
function mapDatabaseError(error: DatabaseError): StorageError {
	switch (error._tag) {
		...
		case "QueryFailed":
			if (error.reason.includes("not found") || error.reason.includes("does not exist")) {
				return { _tag: "NotFound", entity: "Unknown", id: "unknown" };
			}
			if (error.reason.includes("duplicate") || error.reason.includes("unique")) {
...
export class PostgresStorageAdapter implements StoragePort {
	...
	readonly initializeWorkspace = (): Effect.Effect<void, StorageError> =>
		this.db.query("SELECT 1 FROM workspace_config WHERE initialized_at IS NOT NULL LIMIT 1")
			.pipe(
				Effect.asVoid,
	...
	readonly isWorkspaceInitialized = (): Effect.Effect<boolean, StorageError> =>
		this.db.query<{ count: string }>("SELECT COUNT(*) as count FROM workspace_config")
			.pipe(
	...
	readonly getNote = (id: NoteId): Effect.Effect<Note, StorageError> =>
		this.db.query<NoteRow>("SELECT * FROM notes WHERE id = $1", [id])
			.pipe(
				Effect.flatMap(rows => {
					if (rows.length === 0) {
						return Effect.fail({ _tag: "NotFound", entity: "Note", id } as StorageError);
					}
	...
	readonly updateNoteMetadata = (
		...
		this.db.transaction(query =>
			Effect.gen(this, function* () {
				...
				const updated = yield* query<NoteRow>("SELECT * FROM notes WHERE id = $1", [id]);
				
				if (updated.length === 0) {
					yield* Effect.fail({ _tag: "NotFound", entity: "Note", id } as StorageError);
				}
	...
	readonly deleteNote = (id: NoteId): Effect.Effect<void, StorageError> =>
		this.db.query("DELETE FROM notes WHERE id = $1", [id])
			.pipe(
				Effect.asVoid,
				Effect.catchAll(error => Effect.fail(mapDatabaseError(error)))
			);
	...
	readonly saveDraft = (
		...
		Effect.gen(this, function* () {
			...
			yield* this.db.query(
				...
				[request.note_id, request.body_md, JSON.stringify(request.metadata), now]
			);
	...
	readonly getDraft = (note_id: NoteId): Effect.Effect<Draft, StorageError> =>
		this.db.query<DraftRow>("SELECT * FROM drafts WHERE note_id = $1", [note_id])
			.pipe(
				Effect.flatMap(rows => {
					if (rows.length === 0) {
						return Effect.fail({ _tag: "NotFound", entity: "Draft", id: note_id } as StorageError);
					}
	...
	readonly hasDraft = (note_id: NoteId): Effect.Effect<boolean, StorageError> =>
		this.db.query<{ count: string }>("SELECT COUNT(*) as count FROM drafts WHERE note_id = $1", [note_id])
			.pipe(
				Effect.map(rows => Number.parseInt(rows[0].count, 10) > 0),
				Effect.catchAll(error => Effect.fail(mapDatabaseError(error)))
			);
	...
	readonly deleteDraft = (note_id: NoteId): Effect.Effect<void, StorageError> =>
		this.db.query("DELETE FROM drafts WHERE note_id = $1", [note_id])
			.pipe(
				Effect.asVoid,
				Effect.catchAll(error => Effect.fail(mapDatabaseError(error)))
			);
	...
	readonly createVersion = (
		...
		this.db.transaction(query =>
			Effect.gen(this, function* () {
				...
				const now = new Date();
				const content_hash = yield* Effect.promise(() => computeContentHash(content_md));

				// Create version
				yield* query(
					`INSERT INTO versions (id, note_id, content_md, metadata, content_hash, created_at, parent_version_id, label) 
					 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
					[id, note_id, content_md, JSON.stringify(metadata), content_hash, now, parent_version_id, label]
				);
	...
	readonly getVersion = (id: VersionId): Effect.Effect<Version, StorageError> =>
		this.db.query<VersionRow>("SELECT * FROM versions WHERE id = $1", [id])
			.pipe(
				Effect.flatMap(rows => {
					if (rows.length === 0) {
						return Effect.fail({ _tag: "NotFound", entity: "Version", id } as StorageError);
					}
	...
	readonly getCurrentVersion = (note_id: NoteId): Effect.Effect<Version, StorageError> =>
		this.db.query<VersionRow>(
			...
			Effect.flatMap(rows => {
				if (rows.length === 0) {
					return Effect.fail({ _tag: "NotFound", entity: "CurrentVersion", id: note_id } as StorageError);
				}
				return Effect.succeed(mapVersionRow(rows[0]));
			}),
			Effect.catchAll(error => Effect.fail(mapDatabaseError(error)))
		...
		this.db.transaction(query =>
			Effect.gen(this, function* () {
				...
				if (draftRows.length === 0) {
					yield* Effect.fail({ _tag: "NotFound", entity: "Draft", id: request.note_id } as StorageError);
				}
				...
				yield* query(
					`INSERT INTO publications (id, note_id, version_id, published_at, label) 
					 VALUES ($1, $2, $3, $4, $5)`,
					[publicationId, request.note_id, version.id, new Date(), request.label]
				);
		...
		this.db.transaction(query =>
			Effect.gen(this, function* () {
				...
				if (targetRows.length === 0) {
					yield* Effect.fail({ _tag: "NotFound", entity: "Version", id: request.target_version_id } as StorageError);
				}
	...
	readonly getCollection = (id: CollectionId): Effect.Effect<Collection, StorageError> =>
		this.db.query<CollectionRow>("SELECT * FROM collections WHERE id = $1", [id])
			.pipe(
				Effect.flatMap(rows => {
					if (rows.length === 0) {
						return Effect.fail({ _tag: "NotFound", entity: "Collection", id } as StorageError);
					}
	...
	readonly getCollectionByName = (name: string): Effect.Effect<Collection, StorageError> =>
		this.db.query<CollectionRow>("SELECT * FROM collections WHERE LOWER(name) = LOWER($1)", [name])
			.pipe(
				Effect.flatMap(rows => {
					if (rows.length === 0) {
						return Effect.fail({ _tag: "NotFound", entity: "Collection", id: name } as StorageError);
					}
					return Effect.succeed(mapCollectionRow(rows[0]));
				}),
				Effect.catchAll(error => Effect.fail(mapDatabaseError(error)))
	...
	> =>
		this.db.testConnection().pipe(
			Effect.map(() => ({ status: "healthy" as const, details: "PostgreSQL connection active" })),
