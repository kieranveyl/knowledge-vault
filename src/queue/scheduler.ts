...
import { Effect, Queue, Ref, Fiber, Duration } from "effect";
import type { NoteId, VersionId } from "../schema/entities";
...
export type QueueOperation = 
	...
	| { readonly type: "maintenance"; readonly task: () => Effect.Effect<void, never> }
...
export type SchedulerError =
	...
	| { readonly _tag: "ProcessingFailed"; readonly operationId: string; readonly reason: string }
...
export class OperationScheduler {
	...
	constructor(
		...
		private readonly processor: (operation: QueueOperation) => Effect.Effect<void, unknown>,
	) {
		this.config = config;
	...
	readonly submitOperation = (
		...
		Effect.gen(this, function* () {
			...
			const totalQueued = Array.from(state.pendingQueues.values())
	...
	private readonly checkCanProcessMore = (): Effect.Effect<boolean, never> =>
		Ref.get(this.state).pipe(
			Effect.map(state => state.totalInFlight < this.config.maxInFlightPerWorkspace)
		);
	 ...
	 * SPEC: "fair-share across notes"
	 */
	private readonly selectNextItem = (): Effect.Effect<QueueItem | null, never> =>
		Effect.gen(this, function* () {
			const state = yield* Ref.get(this.state);
			...
			yield* Ref.update(this.state, currentState => {
				...
				const itemIndex = noteQueue.findIndex(item => item.priority === bestPriority);
	 ...
	 */
	private readonly startProcessing = (item: QueueItem): Effect.Effect<void, never> =>
		Effect.gen(this, function* () {
			...
			const processingFiber = yield* Effect.fork(
				Effect.gen(this, function* () {
					...
					Effect.ensuring(
						this.markOperationComplete(item.id)
					)
				)
			);
	 ...
	 */
	private readonly markOperationComplete = (operationId: string): Effect.Effect<void, never> =>
			...
			yield* Effect.forever(
				Effect.gen(this, function* () {
					...
					if (now >= state.nextPriorityBoost) {
						...
						yield* Ref.update(this.state, currentState => ({
							...currentState,
							nextPriorityBoost: new Date(now.getTime() + this.config.agingIntervalMs),
						}));
					}
				})
			);
	 ...
	 */
	private readonly boostWaitingItemPriorities = (): Effect.Effect<void, never> =>
		Ref.update(this.state, state => {
			const now = new Date();
	...
	readonly getQueueStatus = (): Effect.Effect<{
		...
		Ref.get(this.state).pipe(
			Effect.map(state => {
				...
				const avgWaitingTimeMs = totalPending > 0 ? totalWaitingTime / totalPending : 0;
